...
public class FuriganaView extends AutoResizeTextView {
        ...
        public void draw(Canvas canvas, float x, float y) {
            ...
            if (m_offset.size() == m_text.size()) {
                ...
                for (int i = 0; i < m_offset.size(); i++)
                    m_text.get(i).draw(canvas, x + m_offset.get(i), y);
            } else {
                // Render with original offsets
                for (TextFurigana text : m_text)
                    text.draw(canvas, x + text.offset_get(), y);
            }
        }
    }
    ...
    protected void onMeasure(int width_ms, int height_ms) {
        ...
        super.setMeasuredDimension(width_ms, height_ms);
    }
    ...
    public void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < m_line_n.size(); i++)
            w += m_line_n.get(i).getWidth();

        float x = getWidth() / 2 - w / 2;
        float y = m_linesize + getHeight() / 2 - m_linesize / 2;

            // Loop lines
        for (int i = 0; i < m_line_n.size(); i++) {
            m_line_n.get(i).draw(canvas, x, y);
            m_line_f.get(i).draw(canvas, x, y - m_height_n);
            y += m_linesize;
        }
    }
    ...
    protected void onTextChanged(final CharSequence text, final int start, final int before, final int after) {
        String t = U.defurigana(text.toString());
        super.onTextChanged(t, start, before, after);
